...
int getConfigFieldIntValue2(const char* strFiledName, const char *config_file_name)
{
	FILE    * config_file;
    ...
    if ( (config_file = fopen(config_file_name, "r")) == NULL) {    
        fprintf(config_file, "reading %s failed.)\n", config_file_name);        
        return -1;
    }   
        ...
        while ( fgets(line, 512, config_file) != NULL ) {
		  ...
		  #define isit2(s) !strcmp(pch=strtok(scrap," \t"),s)
		  if(isit2(strFiledName)){
			  bFound = true;
			  break;
		  }
		}
		...
		if(!bFound  && strcmpi(strFiledName,"vrpn_UDP_PORT") == 0)
			return vrpn_DEFAULT_LISTEN_PORT_NO;
		if(!bFound  && strcmpi(strFiledName,"vrpn_TCP_PORT") == 0)
			return 11111;
		if(!bFound)
			return -1;
...
//fprintf(stderr, "BROKEN - vrpn_Connection::server_check_for_incoming_connections.\n");
	  ...
	  printf("vrpn_client: Connection request received: %s\n", msg);
	  printf("Communication UDP Port: %d\n\n", getConfigFieldIntValue2("vrpn_UDP_PORT"));
