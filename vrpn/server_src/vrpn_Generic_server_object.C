...
int vrpn_Generic_Server_Object::setup_UDP_Button(char * & pch, char * line, FILE * config_file)
{
	char name [LINESIZE];
	...
	{
		fprintf(stderr,"Bad vrpn_UDP_Button line: %s\n",line);
		return -1;
	}
	...
	if ((Keyboards[num_Keyboards] = new vrpn_UDP_Button(name, connection)) == NULL)
	{
		fprintf(stderr,"Can't create new vrpn_UDP_Button\n");
		return -1;
	}
	else
	{
...
int vrpn_Generic_Server_Object::setup_UDP_Tracker (char * & pch, char * line, FILE * config_file) {	
	char s2 [LINESIZE];
	int i1;
	float f1;
	...
	if (sscanf(pch,"%511s%d%g",s2,&i1,&f1) != 3) {
		fprintf(stderr,"Bad vrpn_UDP_Tracker line: %s\n",line);
		return -1;
	}
	...
	// Open the tracker
	if (verbose) printf(
		"Opening vrpn_UDP_Tracker: %s with %d sensors, rate %f\n",
		s2,i1,f1);
	if ( (trackers[num_trackers] =
			new vrpn_UDP_Tracker(s2, connection, i1, f1)) == NULL){
		fprintf(stderr,"Can't create new vrpn_UDP_Tracker\n");
		return -1;
	} else {
		num_trackers++;
	}
...
int vrpn_Generic_Server_Object::setup_Button_SerialMouse (char * & pch, char * line, FILE * config_file) {
    ...
    char s3 [LINESIZE];
    char s4 [32];
    vrpn_UDP_AnalogTYPE mType = MAX_MOUSE_TYPES;
...
//================================
int vrpn_Generic_Server_Object::setup_UDP_Analog (char * & pch, char * line, FILE * config_file) {
    char s2 [LINESIZE];
    vrpn_UDP_Analog * mouse;
    next();

    // Get the arguments (class, mouse_name)
    if (sscanf(pch,"%511s",s2) != 1) {
        fprintf(stderr,"Bad vrpn_UDP_Analog line: %s\n",line);
        return -1;
    }
    ...
    // Open the box
    if (verbose)
        printf("Opening vrpn_UDP_Analog: %s\n",s2);

    try {
	    mouse = new vrpn_UDP_Analog(s2, connection);
    }
    catch (...) {
	fprintf( stderr, "could not create vrpn_UDP_Analog\n" );
#ifdef linux
    ...
    if (NULL == mouse) {
        fprintf(stderr,"Can't create new vrpn_UDP_Analog\n");
        return -1;
    }
...

  {
    ...
    //  whether we should bail.
        ...
        while ( fgets(line, LINESIZE, config_file) != NULL ) {
          ...
          if (isit("vrpn_UDP_PORT")) {
              continue;
          }else if (isit("vrpn_TCP_PORT")) {
              continue;
          } else if (isit("vrpn_UDP_Tracker")) {
              CHECK(setup_UDP_Tracker);
          } else if (isit("vrpn_UDP_Analog")) {
              CHECK(setup_UDP_Analog);
          }else if (isit("vrpn_UDP_Button")) {
            CHECK(setup_UDP_Button);
		  } 
