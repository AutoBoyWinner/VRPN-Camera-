...
//  vrpn_Joywin32 *win32joys[VRPN_GSO_MAX_WIN32JOYS];
  ...
  int		num_posers;
  vrpn_UDP_Analog	* mouses [VRPN_GSO_MAX_MOUSES];
  int		num_mouses;
  vrpn_UDP_Button * Keyboards [VRPN_GSO_MAX_KEYBOARD];
  int		num_Keyboards;
  ...
  int setup_UDP_Tracker (char * & pch, char * line, FILE * config_file);
  ...
  int setup_UDP_Analog (char * & pch, char * line, FILE * config_file);
  ...
  int setup_UDP_Button(char * & pch, char * line, FILE * config_file);
