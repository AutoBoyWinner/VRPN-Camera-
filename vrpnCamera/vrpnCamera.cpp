...
void	VRPN_CALLBACK handle_tracker_pos_quat (void *userdata, const vrpn_TRACKERCB t)
{
	...
	if ( ++t_data->t_counts[t.sensor] >= tracker_stride ) {
		...
		printf("vrpn_UDP_Tracker %s, \nLocation(x,y,z) : (%5.2f, %5.2f, %5.2f)\nRotate Location(RX, RY, RZ) : (%5.2f, %5.2f, %5.2f)\n\n",
		t_data->t_name,
...
void	VRPN_CALLBACK handle_button (void *userdata, const vrpn_BUTTONCB b)
{
    ...
    printf("vrpn_UDP_Button %s, Button%d state : %d\n\n",
...
void	VRPN_CALLBACK handle_analog (void *userdata, const vrpn_ANALOGCB a)
{
    ...
    printf("vrpn_UDP_Analog %s:  (%d chans)\n\n", name, a.num_channel);
    for (i = 0; i < a.num_channel; i++) {
	printf("%dchannel : (%.2f)\n", i, a.channel[i]);
    }
    printf("\n\n");/**/

	/*printf("vrpn_UDP_Analog %s:\n	zoom (%5.2f), focus (%5.2f), fstop (%5.2f)\n\n",
		name, a.channel[0], a.channel[1], a.channel[2]);*/
}
...
"           \n"
"device1[:port] [device2][:port] [device3][:port] [...]\n\n"
...
"  deviceX:  VRPN name of device to connect to (eg: Tracker0@209.85.231.104 or Tracker0@localhost)\n"
"  port: should be same as vrpn_PORT in vrpn.cfg file. \n"
"  (in case of using port, eg: Tracker0@209.85.231.104:1211 or Tracker0@localhost:1211)\n"
...
{
	/*char* str[]={"sfsd", "camera0location@localhost:1211", "camera0lense@localhost:1211", "camera0button@localhost:1211"};
	argv = str;
	argc=4;*/
	//char* str[]={"sfsd", "camera0location@192.168.114.51:1211"};
	//char* str[]={"sfsd", "camera0lense@192.168.114.51:1211"};
	//char* str[]={"sfsd", "camera0button@192.168.114.51:1211"};
	/*argv = str;
	argc=2;*/
